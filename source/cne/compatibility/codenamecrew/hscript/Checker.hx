...
@:allow(cne.compatibility.codenamecrew.hscript.Checker)
class CheckerTypes {
	...
	function addXmlType(x:haxe.rtti.CType.TypeTree,todo:Array<Void->Void>) {
		...
		case TTypedecl(t):
			...
			if( t.path == "cne.compatibility.codenamecrew.hscript.TypeCheck" )
				td.params.reverse();
			todo.push(function() {
...
class Checker {
	...
	public static function typeStr( t : TType ) {
		...
		case TType(t, args):
			if( t.name == "cne.compatibility.codenamecrew.hscript.TypeCheck" )
				typeStr(args[1]);
			else
