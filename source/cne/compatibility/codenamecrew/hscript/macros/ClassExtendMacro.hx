package cne.compatibility.codenamecrew.hscript.macros;

#if macro
import cne.compatibility.codenamecrew.hscript.IHScriptCustomClassBehaviour;
...
class ClassExtendMacro {
	...
	public static function init() {
		...
		for(apply in Config.ALLOWED_CUSTOM_CLASSES) {
			Compiler.addGlobalMetadata(apply, "@:build(cne.compatibility.codenamecrew.hscript.macros.ClassExtendMacro.build())");
		}
	...
	public static function build():Array<Field> {
		...
		if (!cl.name.endsWith("_Impl_") && !cl.name.endsWith(CLASS_SUFFIX) && !cl.name.endsWith("_HSC")) {
			...
			if(fkey == "cne.compatibility.codenamecrew.hscript.CustomClassHandler.TemplateClass") return fields; // Error: Redefined
			if(fkey == "cne.compatibility.codenamecrew.hscript.CustomClassHandler.CustomTemplateClass") return fields; // Error: Redefined
			if(fkey == "cne.compatibility.codenamecrew.hscript.CustomClass") return fields; // Error: Redefined
		...
		shadowClass.kind = TDClass({
			...
			Utils.processImport(imports, "cne.compatibility.codenamecrew.hscript.utils.UnsafeReflect", "UnsafeReflect");
			...
			shadowClass.fields.push({
				name: "__interp",
				pos: Context.currentPos(),
				kind: FVar(macro: cne.compatibility.codenamecrew.hscript.Interp),
				access: [APublic]
			});
			/*
			shadowClass.fields.push({
				...
				macro {
					if (__interp != null) {
						if(__class__fields.contains(name)) {
							...
							if(v != null && v is cne.compatibility.codenamecrew.hscript.Property) 
								return cast(v, cne.compatibility.codenamecrew.hscript.Property).callGetter(name);
							return v;
						}
						else @:privateAccess {
							var cls:cne.compatibility.codenamecrew.hscript.CustomClass = cast __interp.__customClass.__upperClass;
							while(cls != null) {
								if(cls.hasField(name)) 
									return cls.getField(name);
								
								var prev:cne.compatibility.codenamecrew.hscript.CustomClass = cast cls.__upperClass;
								if(prev == null)
									break;
								cls = prev;
							}
				...
				macro {
					if (__interp != null) {
						if(__class__fields.contains(name)) {
							var v:Dynamic = __interp.variables.get(name);
							if(v != null && v is cne.compatibility.codenamecrew.hscript.Property) 
								return cast(v, cne.compatibility.codenamecrew.hscript.Property).callSetter(name, val);
