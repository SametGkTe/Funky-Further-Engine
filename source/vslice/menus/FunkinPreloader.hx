package vslice.menus;

import vslice.funkin.custom.NativeFileSystem;
import vslice.menus.freeplay.DifficultyStars;
#if sys import vslice.menus.components.crash.Logger; #end
			...
			case FunkinPreloaderState.PreloadingPlayAssets:
				if (preloadingPlayAssetsPercent < 0.0)
				{
					...
					FlxG.signals.preStateCreate.add(state ->{
						vslice.funkin.custom.NativeFileSystem.excludePaths.resize(0);
					});
					#end
						...
						for (index => item in assetsToCache)
						{
							try
							{
								...
								if (jsonBlob != null)
								{
									...
									vslice.funkin.FlxAtlasSprite.ANIMATION_OBJECTS.set(item,jsonBlob);
								}
