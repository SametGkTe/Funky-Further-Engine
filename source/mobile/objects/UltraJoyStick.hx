...
/**
 * UltraJoyStick — the new (ArkoseLabs "mobile-controls" based) joystick used
 * by `mobile.MobileControlManager`.
 *
 * Adapted from Psych Engine Online Mobile for Further Engine (no
 * `FunkinFileSystem` dependency).
 */
class UltraJoyStick extends JoyStick {
	//FNF Asset Stuff
	override private function loadObjectGraphic(object:FlxSprite, graphic:String, img:String) {
		...
		#if MODS_ALLOWED
		var modGraphicXml:String = null;
		var modGraphicPng:String = null;
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'mobile/JoyStick/')) {
			var xmlCandidate:String = haxe.io.Path.join([folder, '$fixedModPath.xml']);
			var pngCandidate:String = haxe.io.Path.join([folder, '$fixedModPath.png']);
			if (FileSystem.exists(xmlCandidate) && FileSystem.exists(pngCandidate)) {
				modGraphicXml = xmlCandidate;
				modGraphicPng = pngCandidate;
				break;
			}
		}
		if (modGraphicXml != null && modGraphicPng != null)
