...
class HScript extends SScript
{
	...
	public static function initHaxeModule(parent:FunkinLua)
	{
		...
		{
			trace('initializing haxe interp (SScript) for: ${parent.scriptName}');
	...
	function preset() {
		// SScript preset (0.7.3 Uyumluluğu)
		set('Type', Type);
		#if sys
		set('File', File);
		...
		// 0.7.3 Modları İçin Ekstra Sınıflar (Genişletilmiş)
		...
		// 0.7.3 Modları İçin Yaygın Yardımcı Fonksiyonlar
		set('getTextWidth', function(font:String, text:String, size:Int):Float {
			return CoolUtil.getTextWidth(font, text, size);
		});
		
		set('getColorFromString', function(color:String):Int {
			return CoolUtil.getColorFromString(color);
		});
		
		set('getColorFromHex', function(hex:String):Int {
			return CoolUtil.getColorFromHex(hex);
		});
		
		set('boundTo', function(value:Float, min:Float, max:Float):Float {
			return Math.max(min, Math.min(max, value));
		});

		// 0.7.3 Uyumlu addHaxeLibrary (Haxe içinden de çağrılabilir)
		set('addHaxeLibrary', function(libName:String, ?libPackage:String = '') {
			try {
				var fullName:String = (libPackage != null && libPackage.length > 0) 
					? libPackage + '.' + libName 
					: libName;

				var resolved = Type.resolveClass(fullName);
				if (resolved == null)
					resolved = Type.resolveEnum(fullName);

				if (resolved != null) {
					set(libName, resolved);
					trace('[SScript] addHaxeLibrary başarıyla yüklendi → ' + fullName);
				} else {
					trace('[SScript] addHaxeLibrary BULUNAMADI → ' + fullName);
				}
			} catch(e) {
				trace('[SScript] addHaxeLibrary HATA: ' + e);
			}
		});

		// Lua ile iletişim (0.7.3 tarzı)
		#if LUA_ALLOWED
...
{
	// Mevcut Iris tabanlı kod (orijinal dosya içeriği)
	public var filePath:String;
	public var modFolder:String;
	public var returnValue:Dynamic;
	...
	public static function implement(funk:FunkinLua) {
		...
		funk.addLocalCallback("runHaxeFunction", function(funcToRun:String, ?funcArgs:Array<Dynamic> = null) {
			...
			else
			{
				...
				Iris.error("runHaxeFunction: HScript has not been initialized yet!", pos);
			}
			return null;
		});
